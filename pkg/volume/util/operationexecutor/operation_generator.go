...
func (og *operationGenerator) doOnlineExpansion(volumeToMount VolumeToMount,
	...
	resizeDone, err := og.nodeExpandEciVolume(volumeToMount, actualStateOfWorld, resizeOptions)
	if err != nil {
...
func (og *operationGenerator) nodeExpandEciVolume(
	volumeToMount VolumeToMount,
	actualStateOfWorld ActualStateOfWorldMounterUpdater,
	rsOpts volume.NodeResizeOptions) (bool, error) {
	if !utilfeature.DefaultFeatureGate.Enabled(features.ExpandPersistentVolumes) {
	...
	expandableVolumePlugin, _ :=
		og.volumePluginMgr.FindNodeExpandablePluginBySpec(volumeToMount.VolumeSpec)
	volumeNeedResize, err := actualStateOfWorld.GetVolumeNeedResize(volumeToMount.PodName, volumeToMount.VolumeName)
	if err != nil {
		return false, err
	}
	if volumeNeedResize {
		...
		// File system resize succeeded, now update the PVC's Capacity to match the PV's
		err = actualStateOfWorld.MarkVolumeAsResized(volumeToMount.PodName, volumeToMount.VolumeName)
		if err != nil {
			// On retry, NodeExpandVolume will be called again but do nothing
			return false, fmt.Errorf("MountVolume.NodeExpandVolume update PVC status failed : %v", err)
		}
